/*******************************************************************************#
# # # Withrobot Camera API # # # # Copyright (C) 2016 Withrobot. Inc. # # # #
...
Camera::Camera(const char* dev_name, struct camera_format* conf,
  ...
  if (format_string != 0) {
    ...
    set_format(config.width, config.height, config.pixformat,
               config.rate_numerator, config.rate_denominator);
  }
 ...
 */
void Camera::get_configurations(std::vector<std::string>& formats,
...
bool Camera::get_capability() {
  ...
  DBG_PRINTF("kernel version: %u.%u.%u",
             (v4l2_s.capability.version >> 16) & 0xFF,
             (v4l2_s.capability.version >> 8) & 0xFF,
             v4l2_s.capability.version & 0xFF);
  DBG_PRINTF("Driver: %s", (char*)v4l2_s.capability.driver);
  DBG_PRINTF("Bus: %s", (char*)v4l2_s.capability.bus_info);
  DBG_PRINTF(
      "V4L2_CAP_STREAMING : %s",
      (v4l2_s.capability.capabilities & V4L2_CAP_STREAMING) ? "True" : "False");
  DBG_PRINTF("V4L2_CAP_VIDEO_CAPTURE : %s",
             (v4l2_s.capability.capabilities & V4L2_CAP_VIDEO_CAPTURE)
                 ? "True"
                 : "False");
...
bool Camera::set_buffer() {
  ...
  for (unsigned int i = 0; i < buffer_count; i++) {
    ...
    buffers[i].buffer =
        (unsigned char*)mmap(NULL, v4l2_s.buffer.length, PROT_READ | PROT_WRITE,
                             MAP_SHARED, fd, v4l2_s.buffer.m.offset);
...
bool Camera::enumerate_image_formats(const enum v4l2_buf_type buf_type) {
  ...
  while (xioctl(VIDIOC_ENUM_FMT, &v4l2_s.fmtdesc) != -1) {
    ...
    DBG_PRINTF("Format pixel format: %c, %c, %c, %c",
               (v4l2_s.fmtdesc.pixelformat >> 0) & 0xFF,
               (v4l2_s.fmtdesc.pixelformat >> 8) & 0xFF,
               (v4l2_s.fmtdesc.pixelformat >> 16) & 0xff,
               (v4l2_s.fmtdesc.pixelformat >> 24) & 0xFF);
 ...
 * @return
 */
bool Camera::enumerate_frame_sizes(const unsigned int pixelformat,
  ...
  while (xioctl(VIDIOC_ENUM_FRAMESIZES, &v4l2_s.frmsizeenum) != -1) {
    ...
    switch (v4l2_s.frmsizeenum.type) {
      case V4L2_FRMSIZE_TYPE_DISCRETE:
        DBG_PRINTF("Discrete :: Width: %d, Height: %d",
                   v4l2_s.frmsizeenum.discrete.width,
                   v4l2_s.frmsizeenum.discrete.height);

        desc.clear();
        desc = description +
               (" " +
                Withrobot::to_string<int>(v4l2_s.frmsizeenum.discrete.width) +
                " x " +
                Withrobot::to_string<int>(v4l2_s.frmsizeenum.discrete.height));

        valid_resolution_list[desc] = v4l2_s.frmsizeenum;

        enumerate_frame_intervals(v4l2_s.frmsizeenum.pixel_format,
                                  v4l2_s.frmsizeenum.discrete.width,
                                  v4l2_s.frmsizeenum.discrete.height, desc);

        break;

      case V4L2_FRMSIZE_TYPE_STEPWISE:
        DBG_PRINTF(
            "Stepwise :: Width step(max/min): %d (%d / %d), Height step(max / "
            "min): %d (%d / %d)",
            v4l2_s.frmsizeenum.stepwise.step_width,
            v4l2_s.frmsizeenum.stepwise.max_width,
            v4l2_s.frmsizeenum.stepwise.min_width,
            v4l2_s.frmsizeenum.stepwise.step_height,
            v4l2_s.frmsizeenum.stepwise.max_height,
            v4l2_s.frmsizeenum.stepwise.min_height);
        break;
 ...
 * @return
 */
bool Camera::enumerate_frame_intervals(const unsigned int pixelformat,
  ...
  while (xioctl(VIDIOC_ENUM_FRAMEINTERVALS, &v4l2_s.frmivalenum) != -1) {
    switch (v4l2_s.frmivalenum.type) {
      case V4L2_FRMIVAL_TYPE_DISCRETE:
        ...
        fps = 1.0 / ((double)v4l2_s.frmivalenum.discrete.numerator /
                     (double)v4l2_s.frmivalenum.discrete.denominator);
      ...
      case V4L2_FRMIVAL_TYPE_STEPWISE:
        DBG_PRINTF("Stepwise step (max/min): %.2f (%.2f / %.2f) fps",
                   1.0 / ((double)v4l2_s.frmivalenum.stepwise.step.numerator /
                          (double)v4l2_s.frmivalenum.stepwise.step.denominator),
                   1.0 / ((double)v4l2_s.frmivalenum.stepwise.max.numerator /
                          (double)v4l2_s.frmivalenum.stepwise.max.denominator),
                   1.0 / ((double)v4l2_s.frmivalenum.stepwise.min.numerator /
                          (double)v4l2_s.frmivalenum.stepwise.min.denominator));
        break;
...
int Camera::enumerate_controls() {
  ...
  while (query_ioctl(VIDIOC_QUERYCTRL, &v4l2_s.queryctrl) == 0) {
    if (v4l2_s.queryctrl.flags & V4L2_CTRL_FLAG_DISABLED) continue;

    DBG_PRINTF("id: 0x%X, %s, flags: %d", v4l2_s.queryctrl.id,
               v4l2_s.queryctrl.name, v4l2_s.queryctrl.flags);
...
int Camera::enumerate_control_menu() {
  ...
  for (v4l2_s.querymenu.index = v4l2_s.queryctrl.minimum;
       (int)v4l2_s.querymenu.index <= v4l2_s.queryctrl.maximum;
       v4l2_s.querymenu.index++) {
    if (xioctl(VIDIOC_QUERYMENU, &v4l2_s.querymenu) == 0) {
      DBG_PRINTF("id: %s, 0x%X, %s", v4l2_s.queryctrl.name,
                 v4l2_s.querymenu.index, v4l2_s.querymenu.name);
    } else {
 ...
 */
int Camera::get_frame(unsigned char* out_buffer, const unsigned int size,
 ...
 * @return 목록의 크기
 */
int Camera::get_valid_resolution_list(const char* format_description,
                                      std::vector<const char*>& list) {
  list.clear();

  std::map<std::string, v4l2_frmsizeenum>::iterator it;
  for (it = valid_resolution_list.begin(); it != valid_resolution_list.end();
       ++it) {
 ...
 * @return 목록의 크기
 */
int Camera::get_valid_ratio_list(const char* resolution_description,
                                 std::vector<const char*>& list) {
  list.clear();
 ...
 * @return 목록의 크기
 */
int Camera::valid_controls(
    std::vector<std::pair<const char*, unsigned int>>& list) {
  list.clear();

  std::map<std::string, v4l2_queryctrl>::iterator it;

  try {
    for (it = valid_control_list.begin(); it != valid_control_list.end();
         ++it) {
...
bool Camera::get_control(camera_control& ctrl) {
  ...
  std::map<std::string, v4l2_queryctrl>::iterator it =
      valid_control_list.find(ctrl.name);
...
bool Camera::get_current_format() {
  ...
  DBG_PRINTF("V4L2_CAP_TIMEPERFRAME : %s",
...
bool Camera::get_current_format(camera_format& fmt) {
  ...
  fmt.rate_denominator =
      v4l2_s.streamparm.parm.capture.timeperframe.denominator;
  fmt.frame_rate =
      ((double)fmt.rate_denominator) / ((double)fmt.rate_numerator);
 ...
 * @return
 */
bool Camera::set_format(unsigned int width, unsigned int height,
                        unsigned int pixelformat, unsigned int rate_numerator,
                        unsigned int rate_denomonator) {
  get_current_format();
...
bool Camera::set_format(const char* format_description) {
  ...
  set_format(v4l2_s.frmivalenum.width, v4l2_s.frmivalenum.height,
             v4l2_s.frmivalenum.pixel_format,
             v4l2_s.frmivalenum.discrete.numerator,
             v4l2_s.frmivalenum.discrete.denominator);
...
int Camera::get_buffer(unsigned char* buffer, const unsigned int size) {
  ...
  if (v4l2_s.buffer.bytesused == size ||
      v4l2_s.format.fmt.pix.pixelformat == V4L2_PIX_FMT_MJPEG) {
    retval = v4l2_s.buffer.bytesused;
    memcpy(buffer, buffers[v4l2_s.buffer.index].buffer,
           v4l2_s.buffer.bytesused);
  } else {
    DBG_PRINTF("Warning :: Different buffer size. v4l2: %d, User: %d",
               v4l2_s.buffer.bytesused, size);
  }

  // time stamp
  //    DBG_PRINTF("time stamp : 0x%x, 0x%x, 0x%x, 0x%x",
...
int Camera::xioctl(int IOCTL_X, void* arg) {
  ...
  if (ret && (tries <= 0)) {
    DBG_PRINTF("V4L2_CORE: ioctl (%i) retried %i times - giving up: %s)",
               IOCTL_X, WITHROBOT_CAMERA_IOCTL_RETRY, strerror(errno));
  }
...
int Camera::query_ioctl(int current_ctrl, struct v4l2_queryctrl* ctrl) {
  ...
  /**
   * @todo v4l2_ioctl 로 하는 경우 실패시 무한반복되는 상황이 발생하는것 해결할
   * 것
   */
  int ret = 0;
  int tries = 4;
