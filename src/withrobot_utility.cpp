/*******************************************************************************#
# # # Withrobot Utilities # # # # Copyright (C) 2015 Withrobot. Inc. # # # #
 ...
 */
int Withrobot::get_usb_device_info_list(
...
Mutex::Mutex() {
  ...
  if (res != 0) {
    ...
    throw(WithRobotException("pthread_mutexattr_init returns " +
                             std::string(str_res)));
  }
  ...
  if (res != 0) {
    sprintf(str_res, "%d", res);
    throw(WithRobotException("pthread_mutexattr_settype returns " +
                             std::string(str_res)));
  }
  ...
  if (res != 0) {
    sprintf(str_res, "%d", res);
    throw(WithRobotException("pthread_mutex_init returns " +
                             std::string(str_res)));
  }
  ...
  if (res != 0) {
    sprintf(str_res, "%d", res);
    throw(WithRobotException("pthread_mutexattr_destroy returns " +
                             std::string(str_res)));
  }
}
...
Timer::Timer(std::string name, unsigned int max_cnt)
    : name(name), cnt(0), max_cnt(max_cnt), elapsed_sum(0), elapsed_avg(0) {
  if (max_cnt < 1) {
    max_cnt = 1;
  }
