...
struct camera_control {
  ...
  void dbg_print() {
    DBG_PRINTF(
        "Id: %u, Name: %s, Value(default [min, step, max]): %d ( %d [%d, %d, "
        "%d] ), Type: %d, Flag: %d",
        id, name.c_str(), value, default_value, minimum, step, maximum, type,
        flags);
...
class Camera {
 public:
   ...
   *
   *  [1] "Exposure (Absolute)", Value(default [min, step, max]): 39 ( 39 [1, 1,
   *625] ) [2] "Brightness",          Value(default [min, step, max]): 64 ( 64
   *[0, 1, 127] )  // gain
   *
   */
  Camera(const char* dev_name, struct camera_format* conf = 0,
